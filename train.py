...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.data_name == 'cifar10': 
        data_size = 50020
    if args.data_name == 'imagenet100' or args.data_name == 'imagenet50': 
        data_size = 129395+1
    ...
    logdir = '20221005_%s_%s_%s-%s-%s_bz_%s_E%s_WR%s_lr_%.3f_%s_wd_%s_t_%s_g_%s_%s_1'%(args.data_name, args.arch, method_name, args.dim, args.mlp_dim, global_batch_size, args.epochs, args.warmup_epochs, args.lr, args.learning_rate_scaling, args.weight_decay, args.t, args.gamma, args.optimizer )
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if len(best_model.keys()) != 0 and ((epoch+1)%50 == 0 or epoch == args.epochs+1):
...
def train(train_loader, model, optimizer, scaler, summary_writer, epoch, args):
    ...
    for i, (images, labels, index) in enumerate(train_loader):
        ...
        with torch.cuda.amp.autocast(True):
            loss = model(images[0], images[1], labels, index, args.gamma)
